...
class Lemonwaymkt extends Module{
	...
	public function hookActionOrderPaymentHook($params){
		...
		$transactions = array();
		$seller_cart_products = $params['customervar'];
		
		$first_p = current($seller_cart_products);
		
		$id_order = $first_p['id_order'];
		
		if(!$id_order)
			return '';
		

		//check if payment method is LW
		...
		foreach ($seller_cart_products as $product) {
			...
			$id_customer = $product['id_customer'];
			Logger::AddLog("Id cus: ".$id_customer,1,null,null,true);
			$obj_mpcommission = new MarketplaceCommision();
			$obj_mpcommission->customer_id = $id_customer;
			$commission_by_seller = $obj_mpcommission->getCommissionRateBySeller();
			//apply global commission, if commission by particular seller not defined and if commission set to 0.00 no commission applied for this seller
			...
			Logger::AddLog("Com rate: ".$commission_rate,1,null,null,true);
			// create seller order commission details
			$admin_commission = (($product['total_price_tax_excl']) * $commission_rate) / 100;
			...
			//Distribution of product tax close
			Logger::AddLog("Admin com: ".$admin_commission,1,null,null,true);
			Logger::AddLog("total tax: ".$total_tax,1,null,null,true);
			Logger::AddLog("sellet amount: ".$seller_amt,1,null,null,true);
			
			if(!isset($transactions[$id_customer]))
			{
				Logger::AddLog("before create new transac",1,null,null,true);
				try {
					...
					$wallet_transac->seller_id = $product['marketplace_seller_id'];
					$wallet_transac->shop_name = $product['shop_name'];
					$wallet_transac->credit_wallet = $wallet ? $wallet->id_lw_wallet : "";
					$wallet_transac->debit_wallet = LemonWayConfig::getWalletMerchantId();
					$wallet_transac->admin_commission = $admin_commission;
					$wallet_transac->amount_to_pay = $seller_amt;
					$wallet_transac->amount_total = $admin_commission + $seller_amt;
					Logger::AddLog("after set values");
		...
		Logger::AddLog(count($transactions)." transactions",4,null,null,true);
		foreach ($transactions as $customer_id=>$w_transac){
			...
			if($res)
			{
				Logger::AddLog("Order State: ".$order->current_state,4,null,null,true);
				Logger::AddLog("Transac saved with id: ".$w_transac->id,4,null,null,true);
				Logger::AddLog("Status id: ".$order->getCurrentOrderState()->id,4,null,null,true);
				Logger::AddLog("Status success: ".Configuration::get('PS_OS_PAYMENT'),4,null,null,true);
				
				//Call send Payment method if config is auto dispatch
				//Check conf auto dispatch and Check status order and if it's valid order we do send payment
				if(Configuration::get('LEMONWAYMKT_AUTO_DISPATCH')/* && ($order->getCurrentOrderState()->id == Configuration::get('PS_OS_PAYMENT'))*/) { 
					
					$params = array(
							"debitWallet"	=> $w_transac->debit_wallet,
							"creditWallet"	=> $w_transac->credit_wallet,
							"amount"		=> $w_transac->amount_to_pay,
							"message"		=> sprintf($this->l('Send payment for order %s'),array($id_order)),
							//"scheduledDate" => "",
							//"privateData"	=> "",
					);
